  ...
  useEffect(() => {
    const fetchOrderAndRedirect = async () => {
      try {
        ...
        if (res.data) {
          setOrder(res.data);
          // Redirect to Stripe or PayFast
          const paymentRes = await axios.post(`${API_URL}/payments/create-checkout-session`, {
            orderId: orderId,
            amount: res.data.totalPrice || res.data.totalAmount,
          ...
          if (paymentRes.data.url) {
            window.location.href = paymentRes.data.url; // redirect to Stripe/PayFast
          }
        }
      } catch (err) {
    ...
    if (orderId) fetchOrderAndRedirect();
  }, [orderId]);
